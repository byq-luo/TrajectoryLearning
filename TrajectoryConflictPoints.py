...
class AssignBrokenTrajectoryLabels(LoadSave):
    ...
    def validate(self, index=None):
        ...
        for ind, item in enumerate(index):
            plt.plot(self._trajDataBroken[item]["X"], self._trajDataBroken[item]["Y"], "ro", markersize=4)
            ...
            plt.plot(self._averageRoutes[featureLabel]["X"], self._averageRoutes[featureLabel]["Y"], "g-s", markersize=6)
        l = plt.legend(["BrokenTrajectory", "Routes"], loc=2, fontsize='xx-large')
...
trajDataFeaturesBroken = load_data("..//Data//TrajectoryDataFeaturesBroken.pkl")
...
clf.validate(index=trajDataBrokenKeys[120:125])
'''
###############################################################################
...
clf = ConflictRoutes(averageRoutes=averageRoutes,
                     trajDataFeaturesComplete=trajDataFeaturesComplete,
